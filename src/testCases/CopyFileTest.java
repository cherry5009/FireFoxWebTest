...
public class CopyFileTest {
	...
	//从根目录向下复制一级目录
	//把testFile.doc复制到testFolder1中去，结构变成test,1->test,2->3
	@Test(priority=0)
	public void test0() {
		...
		Assert.assertEquals(true, copyList.confirmCopyOneLevelIn("testFile.doc", "testFolder1"));
	}
	
	//复制到父文件夹的兄弟文件夹的子文件夹中
	//把testFile.doc复制到testFolder3中，结构变成test,1->test,2->3->test
	@Test(priority=1)
	public void test1(){
		...
		Assert.assertEquals(true, copyList.confirmCopyTwoLevelIn("testFile.doc", "testFolder2", "testFolder3"));  //有重名，后面加了（1）
	}
	
	//复制到父文件夹中去
	//把testFile.doc复制到testFolder2中，结构变成test,1->test,2->(3->test,test)
	@Test(priority=2)
	public void test2(){
		//先进入到testFile.doc所在的位置
		...
		Assert.assertEquals(true, copyList.confirmCopyOneLevelIn("testFile.doc", "testFolder2"));
	}
	
	//复制到根目录去
	//先删掉原来根目录下存在的testFile.doc，再把testFile.doc复制到根目录下，结构变成test,1->test,2->(3->test,test)
	@Test(priority=3)
	public void test3(){
		//先删掉根目录下最初存在的testFile.doc
		ActionList oldFolder = new ActionList(driver, "testFile.doc");
		oldFolder.initActionList();
		oldFolder.clickDelete();
		Assert.assertEquals(true, oldFolder.confirmDelete());
		//进入到testFile.doc所在的位置
		...
		CopyList copyList = new CopyList(driver);
		copyList.copyToRoot();
		Assert.assertEquals(true, copyList.confirmCopyToRoot("testFile.doc"));
	}
	
	//复制两级目录
	//先删掉原来testFolder3下的testFile.doc，再把testFile.doc复制到testFolder3下，结构变成test,1->test,2->(3->test,test)
	@Test(priority=4)
	public void test4(){
		//先删掉testFolder3下原来存在的testFile.doc
		new WebDriverWait(driver, 5).until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("div[title=\"testFolder2\"]"))).click();
		new WebDriverWait(driver, 5).until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("div[title=\"testFolder3\"]"))).click();
		ActionList oldFolder = new ActionList(driver, "testFile.doc");
		oldFolder.initActionList();
		...
		CopyList copyList = new CopyList(driver);
		copyList.copyTwoLevelIn("testFolder2", "testFolder3");
		Assert.assertEquals(true, copyList.confirmCopyTwoLevelIn("testFile.doc", "testFolder2", "testFolder3"));
	}
	
	//复制两级目录，复制到根目录下
	//先删掉根目录下的testFile.doc，再把testFile.doc复制到根目录下，结构变成test,1->test,2->(3->test,test)
	@Test(priority=5)
	public void test5(){
		//先删掉根目录下原来存在的testFile.doc
		ActionList oldFolder = new ActionList(driver, "testFile.doc");
		oldFolder.initActionList();
		oldFolder.clickDelete();
		Assert.assertEquals(true, oldFolder.confirmDelete());
		//进入到要复制的testFile.doc所在的位置
		new WebDriverWait(driver, 3).until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("a[title=\"亿方云\"]"))).click();
		new WebDriverWait(driver, 5).until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("div[title=\"testFolder2\"]"))).click();
		new WebDriverWait(driver, 5).until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("div[title=\"testFolder3\"]"))).click();
		ActionList objActionList = new ActionList(driver, "testFile.doc");
	...
	public void afterTest(){
		//reset data
		//delete the testFolder1 and 2.Because test,1->test,2->(test,3->test)
